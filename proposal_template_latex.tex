...
\rhead{\small \textbf{Tody —— Your Desktop Companion}}
...
\usepackage{xltabular} %进阶版？？？

%% =========================
%% Timeline Setup (Add to Preamble)
%% =========================
\usepackage{environ}
\usepackage{tikz}
...
\NewEnviron{vtimeline}[1][]{%
    ...
    \begin{tikzpicture}[column 1/.style={anchor=east},
        column 2/.style={anchor=west, text width=14cm, align=left}, % 调整了文字宽度以适应页面
...
本产品（Tody）是一款致力于将日程与待办管理轻松化的桌面弹窗软件。产品将日程管理系统与当下流行的桌面宠物系统相结合，旨在消解传统待办管理的刻板与严肃的同时更好地帮助用户达成短期与长期的目标。产品主要包括每日待办管理；短期与长期目标管理；桌面宠物陪伴互动；宠物皮肤收集切换等功能。桌面宠物概念的加入会在一定层面上成为督促与提醒用户实时完成任务的标识；提升趣味度并降低用户对于待办管理程序提醒的抵触感；增强产品与用户的温度与陪伴感；并且在用户完成待办项目时给予成就感以达成良性循环。该产品主要面向年轻、精通技术的职场人士，尤其适合大学生和远程工作者使用。
...
\begin{itemize}
    ...
    2.潜在用户端：喜爱简约长时陪伴工作群体，ACGN文化喜好者

    \item \textbf{Goals:} 1.核心功能：构建一个稳定易用的系统，无缝融合任务管理（创建，完成，追踪）与虚拟宠物养成（皮肤解锁，状态互动，情绪价值提供）两大核心模块。
    2.提升动力：通过皮肤奖励与电子宠物互动反馈机制，预计显著提高用户完成日常任务的意愿与持续性。\\
    3.增强长期参与度：通过收集系统、成就体系，培养用户长期使用习惯，构成同好社交圈子。\\
...
在这里，我们提出了“桌面宠物生产力助手”的需求架构。该项目旨在通过将任务管理逻辑与情感交互引擎相结合，解决用户在长时间办公或学习中产生的动力缺失问题。
...
\begin{itemize}
    ...
    用户需要一个集中式的、非侵入式的桌面界面来管理每日任务。与传统的手机端 App 不同，我们的悬浮窗可以常驻桌面，使用户在不切换窗口、不中断工作流的情况下，即可快速查看、记录并汇报任务进度，除了快速查看任务外，用户期望利用 PC 端特性，通过拖拽文件等直观操作与桌宠互动，简化任务创建流程，从而维持“心流”状态。 
    ...
    除了基本的实用功能外，用户还希望获得一个具有“生命感”的伙伴。该系统应当提供一个虚拟的角色，能够根据用户的规划完成程度动态调整，在用户拖延（如长时间没有操作）时，虚拟宠物能通过动态反馈（如文字激励或情绪表达）提供陪伴感，模拟一种“并肩作战”的氛围。\\
...
\begin{itemize}
   ...
   系统应内置一套奖励算法，当任务状态变更时，系统需自动结算奖励。计算公式为：$$奖励值 = 基础分 \times 任务权重 \times 完成效率$$系统应包含一个“装扮/商店”模块，允许用户消耗已获得的虚拟货币来购买不同的皮肤、配饰或环境主题，并实时更新桌面宠物的渲染模型。\\
...
\begin{itemize}
    ...
    应用程序必须使用无边框、透明背景的浮窗形式。它必须支持“点击穿透”模式或“透明度动态调节”，以确保在不与宠物交互且宠物可见的情况下，它不会遮挡用户与IDE 或办公软件（例如集成开发环境或文字处理软件）进行交互。
   ...
   系统必须确保用户数据的安全。所有任务数据、奖励记录和宠物状态应实时保存至本地轻量级数据库（如 SQLite）中不会被删除。在程序意外关闭或系统重启后，宠物的所有状态和待办事项列的所有进度必须在下次启动时应完整恢复。 
\end{itemize}

\section{Functional Decomposition}

为了确保系统的可维护性与扩展性，本项目采用了模块化的设计思路，将复杂的桌面宠物系统分解为三个互相关联的核心子系统。

\subsection{数据持久层 (Data Persistence Layer)} 
该层级负责系统中所有静态与动态数据的存储与管理，是整个软件的底层支撑。
...
\subsection{系统逻辑层 (System Logic Layer)} 
作为系统的“大脑”，该层级负责处理用户行为并决策宠物的反应，连接了数据与视觉表现。 
...
\begin{itemize} 
   ...
   \item \textbf{日历日程记录（Calendar \& Schedule Recorder）：}作为系统逻辑层中\textbf{时间维度核心管理组件}，承担全局时间基准维护、日程时序运算、任务时间管理及时间数据联动交互职责，是连接数据持久层、专注计算引擎、行为决策引擎与 UI 时间可视化的关键枢纽，保障系统内所有时间相关逻辑的统一、准确与可追溯。
   
    \item \textbf{时间基准与日程元数据同步：}双向对接数据持久层的任务存储数据库，负责读写任务创建时间、截止时间、重复周期（每日 / 每周 / 自定义循环）、计划专注时段等时间类元数据，维护系统全局统一时间轴，确保各逻辑组件时间基准一致，杜绝多模块时间计算偏差。 
    
    \item \textbf{任务时序计算与时效预判：}自动运算任务时间序列、剩余有效时长、逾期临界节点，识别多任务时间冲突、专注时段与日程重叠等异常状态，实时向行为决策引擎输出「任务即将逾期」「时间紧张」「日程空闲」等时间状态信号，为随机事件触发、宠物状态决策提供核心时间维度依据。 
    
    \item \textbf{专注时段联动与时间统计：}对接专注与数值计算引擎，同步番茄钟专注时段、有效专注时长、专注中断记录，按日 / 周 / 月维度聚合统计用户专注时长分布、任务完成时间规律、日程执行率，形成标准化时间统计数据，回传数据层完成持久化存档。 
\end{itemize}

\subsection{前端层/UI层 (Front-end layer/UI layer)} 
该层级直接与用户进行交互，负责将逻辑层生成的抽象指令转化为具象的视觉效果。 

\begin{itemize} 
   \item \textbf{透明窗口渲染引擎：} 可以无边框浮窗的置顶显示、透明度调节以及“点击穿透”逻辑，确保其不干扰主程序工作。 

   \item \textbf{状态显示机：}管理多套状态序列（如：普通、专注工作、生病、离家出走后的空白占位）。 并进行图像切换。它包含“呼吸”、“行走”、“欢呼”、“委屈”等多种画面状态，并根据逻辑层发出的状态指令平滑地进行画面过渡。 
...
\textcolor{red}{这个图等前面换成英文要排下版}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{normal flow.png}
    \caption{Diagram of normal flow}
    \label{normal flow}
\end{figure}
...
\subsection*{Architecture Overview(Fig.\ref{软件系统架构图})}
...
\subsection*{Tech Stack(Fig.\ref{Tech Stack})}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{Tech Stack(1).png}
    \label{Tech Stack}
    \caption{Tech Stack}
\end{figure}

\subsection*{Major Components(Fig.\ref{fig:T逻辑框架}) and (Fig.\ref{fig:用户侧})}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{用户侧.png}
    \caption{User Action Layer}
    \label{fig:用户侧}
\end{figure}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{逻辑框架.png}
    \caption{Main Process \& Logic Layer}
    \label{fig:T逻辑框架}
\end{figure}
...
% =========================
\section{Risk Analysis and Project Plan}
...
\begin{itemize}
    ...
    \item \textbf{Risk 1:} 在不同的操作系统端也可能出现不兼容问题 \\
    \textit{Impact:} 导致系统适用范围缩减，部分用户无法正常使用核心范围 \\
    \textit{Mitigation:} 采用跨平台框架，建立多系统测试环境进行同步开发与测试
    
    \vspace{0.5em} % Extra space between risks
    
    \item \textbf{Risk 2:} 开发过程中主要目标可能逐渐偏离，使得系统过于杂乱 \\
    \textit{Impact:} 造成架构混乱，效率降低的同时增加了维护成本 \\
    \textit{Mitigation:} 明确核心需求边界，定期评测需求变更，进行项目对齐
    
    \vspace{0.5em} % Extra space between risks
    
    \item \textbf{Risk 3:} 由于交互系统较为复杂测试压力会指数增长，可能会出现不可遇见的交互错误 \\
    \textit{Impact:} 增加测试修复成本并延误上线 \\
    \textit{Mitigation:} 采用自动化测试，并模拟真实用户场景进行专项测试

    \vspace{0.5em} % Extra space between risks
    
    \item \textbf{Risk 4:} 项目拥有的正反馈机制无法正确激励用户，反而增大用户使用压力 \\
    \textit{Impact:} 降低用户积极性与留存率，违背初衷\\
    \textit{Mitigation:} 通过前期用户调研设计多样化反馈机制，并进行小范围测试调整
    
    \vspace{0.5em} % Extra space between risks
    
    \item \textbf{Risk 5:} 桌面系统功能过于杂乱分散用户办公注意力 \\
    \textit{Impact:} 降低用户办公效率，流失客户满意度\\
    \textit{Mitigation:} 聚焦核心功能，精简前端面板
    
    \vspace{0.5em} % Extra space between risks
    
    \item \textbf{Risk 6:} 用户私密日程信息保护问题 \\
    \textit{Impact:} 引发用户隐私泄露与信任危机\\
    \textit{Mitigation:} 对敏感信息进行加密存储，实施严格访问权限控制
    
    \vspace{0.5em} % Extra space between risks
    
    \item \textbf{Risk 7:} 无法拓宽除固定受众外的群体 \\
    \textit{Impact:} 限制用户增长与市场份额，影响项目长期发展 \\
    \textit{Mitigation:} 调研潜在用户需求，通过精准营销拓宽受众
    
    \vspace{0.5em} % Extra space between risks
    
    \item \textbf{Risk 8:} 相似差异点会被市场迅速赶超覆盖 \\
    \textit{Impact:} 削弱产品竞争力，市场空间被挤占\\
    \textit{Mitigation:} 持续关注市场动态并快速迭代，以创新构建独特竞争力
\end{itemize}
...
% --- Project Plan 表格 (可跨页版本) ---

% 调整行高
\renewcommand{\arraystretch}{1.2}

% 开始 xltabular 环境 (注意：不要包裹在 table 环境中)
% 参数 1: 总宽度 (\textwidth)
% 参数 2: 列格式 (保持你之前的比例设置 0.5 + 0.8 + 1.7 = 3)
\begin{xltabular}{\textwidth}{| 
    ...
    % --- 1. 定义第一页的表头 ---
    \caption{Project Milestones and Timeline} \\ % 如果需要标题，写在这里
    \hline
    \textbf{Time Period} & \textbf{Milestone} & \textbf{Outcome / Deliverable} \\
    \hline
    \endfirsthead

    % --- 2. 定义后续页面的表头 (翻页后显示的标题) ---
...
\subsubsection*{Project Timeline Visualization}
\begin{center}
    % 使用 primaryBlue 保持风格统一
    \begin{vtimeline}[timeline color=primaryBlue, add bottom line, line offset=2pt]
        Week 1 & \textbf{Requirement \& Style}: Refine MVP requirements and confirm art style\endlr
        Week 2 & \textbf{Prototype \& Logic}: UI/UX prototypes and logical framework design\endlr
        Week 3 & \textbf{Environment Setup}: Dev environment and basic technical frameworks\endlr
        Week 4 & \textbf{Core Interaction}: Front-end/back-end logic and achievement functions\endlr
        Week 5 & \textbf{Special Functions}: Floating window, animations, and skin system\endlr
        Week 6 & \textbf{Data Storage}: Local data persistence development\endlr
        Week 7 & \textbf{Testing}: Functional and basic performance testing\endlr
        Week 8 & \textbf{Optimization}: Bug fixes and parameter tuning\endlr
        Week 9 & \textbf{Final Inspection}: Security check and promotion preparation\endlr
        Week 10 & \textbf{Review \& Planning}: Project review and advanced feature outlook\endlr
    \end{vtimeline}
\end{center}
