...
\begin{itemize}
    ...
    \item \textbf{Goals:} 1.核心功能：构建一个稳定易用的系统，无缝融合任务管理（创建，完成，追踪）与虚拟宠物养成（皮肤解锁，状态互动，情绪价值提供）两大核心模块。\textcolor{red}{更多可以说一下效率方面的：日历、日程生成，标记完成……}\\
    2.提升动力：通过皮肤奖励与电子宠物互动反馈机制，预计显著提高用户完成日常任务的意愿与持续性。\\
    3.增强长期参与度：通过收集系统、成就体系和潜在的\textcolor{red}{？？？}社交功能，培养用户长期使用习惯，提高产品粘性。\\
...
\subsection{系统逻辑层 (System Logic Layer)} 作为系统的“大脑”，该层级负责处理用户行为并决策宠物的反应，连接了数据与视觉表现。 
...
\begin{itemize} 
   ...
   \item \textbf{\textcolor{red}{日历日程记录}：负责与数据层交互，记录计算日程时间……}
\end{itemize}

\subsection{前端层/UI层 (Front-end layer/UI layer)} 该层级直接与用户进行交互，负责将逻辑层生成的抽象指令转化为具象的视觉效果。 
\begin{itemize} 
...
\textbf{Normal Flow}\textcolor{red}{情景一完美！！}
...
\begin{figure}[htbp]
    ...
    \caption{Flowchart of normal flow}
    \label{normal flow}
\end{figure}
...
\begin{itemize}
    ...
    \textit{Impact:} 导致功能系统不稳定以及后期维护成本上升\\
    \textit{Mitigation:} 提前查看组件兼容性历史，锁定稳定版本并定期测试兼容性
